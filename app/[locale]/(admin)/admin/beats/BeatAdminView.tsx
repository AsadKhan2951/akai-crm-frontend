    ...
    <div className="space-y-6">
      ...
      {canManage ? (
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm md:p-6">
          ...
          {data.agents.length === 0 ? <p className="mt-3 text-sm text-[#b42318]">{t("noAgents")}</p> : null}
        </section>
      ) : null}
      ...
      {message ? <p role="status" className="text-sm font-medium text-primary">{message}</p> : null}{error ? <p role="alert" className="text-sm font-medium text-[#b42318]">{error}</p> : null}
    </div>
  );
}
